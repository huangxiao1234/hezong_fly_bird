...
Page({
  ...
  getbirdtop:function(e){
    birdtop=e.touches['0']['clientY']
  ...
  panduan() {
    console.log(clientX)
    if (birdtop < pipeheight - 10 & pipeleft <= clientX & (pipeleft+30) >= clientX) {
      clearInterval(downinterval);
      clearInterval(flyinterval);
      clearInterval(pipemoveinterval);
      this.setData({ endhidden: "", restarthidden: "" })
    }
    if (birdtop > 600 - pipe11height - 45 & pipeleft <= clientX & (pipeleft+30) >= clientX) {
      clearInterval(downinterval);
      clearInterval(flyinterval);
      clearInterval(pipemoveinterval);
      this.setData({ endhidden: "", restarthidden: "" })
    }
    if (birdtop < pipe2height - 10 & pipeleft2 <= clientX & pipeleft2 >= (clientX-30)) {
      clearInterval(downinterval);
      clearInterval(flyinterval);
      clearInterval(pipemoveinterval);
      this.setData({ endhidden: "", restarthidden: "" })
    }
    if (birdtop > 600 - pipe22height - 45 & pipeleft22 <= clientX & pipeleft22 >= (clientX-30)) {
      clearInterval(downinterval);
      clearInterval(flyinterval);
      clearInterval(pipemoveinterval);
      this.setData({ endhidden: "", restarthidden: "" })
